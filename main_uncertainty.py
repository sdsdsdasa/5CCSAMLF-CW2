"""
TPC RP — Uncertainty-Weighted Typicality modification (Task 3).

Identical to main.py except the query score is:
    score(x) = typicality(x) × normalised_entropy(classifier(x))

On iteration 1 there is no previous classifier, so pure typicality is used
(same as the original). From iteration 2 onwards, the previous iteration's
classifier provides per-sample prediction entropy which is normalised to [0,1]
and multiplied with the typicality score before ranking candidates.

Saves results to results/uncertainty/:
    results_uncertainty.csv
    accuracy_curve_uncertainty.png
"""
...
def main():
    ...
    SIMCLR_EPOCHS   = 50              # reduced for speed (set to 500 for full run)
    CLF_EPOCHS      = 20              # reduced for speed (set to 200 for full run)
    SEED            = 42
    DATA_DIR        = './data'
    ...
    print("Modification: uncertainty-weighted typicality (score = typicality × entropy)")
    ...
    if os.path.exists(orig_csv):
        ...
        plt.plot(orig_n, orig_acc, marker='s', linewidth=2,
                 linestyle='--', label='TPC RP (original, pure typicality)')

    plt.plot(n_labeled, accuracies, marker='o', linewidth=2,
             label='TPC RP (uncertainty-weighted typicality)')
    plt.xlabel('Number of Labeled Examples')
    plt.ylabel('Test Accuracy (%)')
    plt.title('TPC RP: Original vs Uncertainty-Weighted Typicality on CIFAR-10')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plot_path = os.path.join(OUT_DIR, 'accuracy_curve_uncertainty.png')
    plt.savefig(plot_path, dpi=150)
    print(f"Plot saved to {plot_path}")

    # Print comparison table
    print("\n=== RESULTS SUMMARY ===")
    print(f"{'|L|':>5}  {'Unc-Weighted':>14}  {'Original':>10}  {'Delta':>8}")
    orig_map = {}
    if os.path.exists(orig_csv):
        import csv as csv_mod
        with open(orig_csv) as f:
    ...
    for r in results:
        ...
        delta = uw - ori if nl in orig_map else float('nan')
        print(f"{nl:>5}  {uw:>13.2f}%  {ori:>9.2f}%  {delta:>+7.2f} pp")
